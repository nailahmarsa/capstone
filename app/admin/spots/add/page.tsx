...
export default function AddSpotPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Kategori disamakan dengan spotType untuk simplifikasi
      ...
      const res = await fetch(`${apiUrl}/admins/spots`, {
        ...
        headers: {
          ...
          // JANGAN set Content-Type ke application/json saat pakai FormData
        },
        body: submitData,
      });
      ...
      if (res.ok) {
        ...
        router.push("/admin/dashboard"); // Kembali ke panel admin
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#FBF2F3] font-['DM_Sans'] pb-20">
      {/* Header */}
      <div className="bg-[#2f4b2f] px-6 py-4 flex items-center gap-4 text-white rounded-b-[32px] shadow-md sticky top-0 z-50">
        <button onClick={() => router.back()} className="p-2 hover:bg-white/20 rounded-full transition-colors">
          <ArrowLeft size={20} />
        </button>
        <h1 className="text-lg font-bold">Tambah Spot Baru</h1>
      </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[32px] shadow-sm flex flex-col gap-6 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input required type="text" name="name" value={formData.name} onChange={handleChange} className="w-full bg-[#f5f5f5] px-4 py-3 rounded-xl outline-none text-sm" placeholder="Contoh: Erasmus Huis" />
            </div>
            <div>
          ...
          <div>
            ...
            <textarea name="address" value={formData.address} onChange={handleChange} className="w-full bg-[#f5f5f5] px-4 py-3 rounded-xl outline-none text-sm resize-none h-20" placeholder="Alamat lengkap tempat..." />
          </div>

          <div>
            <label className="block text-sm font-bold text-[#2f4b2f] mb-2">Deskripsi</label>
            <textarea name="description" value={formData.description} onChange={handleChange} className="w-full bg-[#f5f5f5] px-4 py-3 rounded-xl outline-none text-sm resize-none h-24" placeholder="Ceritakan suasana tempat ini..." />
          </div>

          <div>
            <label className="block text-sm font-bold text-[#2f4b2f] mb-2">Fasilitas Tambahan</label>
            <input type="text" name="facilities" value={formData.facilities} onChange={handleChange} className="w-full bg-[#f5f5f5] px-4 py-3 rounded-xl outline-none text-sm" placeholder="Contoh: WiFi, Colokan, AC, Mushola" />
          </div>

          {/* Kategori (Tags) - Wajib Diisi karena dipakai untuk Filtering */}
          <div className="border-t border-gray-100 pt-6 mt-2">
            <h3 className="text-sm font-bold text-[#2f4b2f] mb-4">Kategorisasi (Penting untuk Search) *</h3>
          ...
          <button 
            ...
            className="mt-6 flex justify-center items-center gap-2 bg-[#2f4b2f] text-white font-bold py-4 rounded-xl hover:bg-[#3d6b3d] disabled:opacity-50 transition-colors"
          >
            {isSubmitting ? "Menyimpan..." : (
              <>
                <Upload size={18} /> Simpan Spot
              </>
            )}
          </button>
        </form>
